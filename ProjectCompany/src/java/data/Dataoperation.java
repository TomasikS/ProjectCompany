...
conn.close();
   ...
   public static List<Employee>  getAllEmployeesId(){
        List<Employee> list = new ArrayList<>();
        try {
            Class.forName(driver).newInstance();
            conn = DriverManager.getConnection(url,username,password);
...
conn.close();
    ...
    public static int save(Employee e){  
       ...
       List<Employee> list=getAllEmployeesId();
       pom=list.get(list.size()-1).getId();
       
        try{  
                 Class.forName(driver).newInstance();
            conn = DriverManager.getConnection(url,username,password);
            PreparedStatement ps=(PreparedStatement) conn.prepareStatement(  
                         "insert into employee_details(idu,amount,kids,hours) values (?,?,?,?)");  
            
            ps.setInt(1,pom); 
            ps.setInt(2,e.getAmount());  
            ps.setInt(3,e.getKids());  
            ps.setInt(4,e.getHours());  
      ...
      public static List compute(){  
        ...
        for (int i=0;i<zozv.size();i++)
            {
             zozv.get(i).plat=  zozv.get(i).getAmount()*zozv.get(i).getHours()+zozv.get(i).getBonus()+ zozv.get(i).hplat;
